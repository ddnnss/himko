...
def kv(request):
    ...
    send_mail(f'Заполнен квиз на сайте http://{request.GET.get("type")}.ru', None, 'info@specsintez-pro.ru',
              ['igor@astrapromo.ru'],
...
def kv1(request):
    ...
    send_mail(f'Заполнен квиз ', None, 'info@specsintez-pro.ru',
              ['igor@astrapromo.ru'],
              fail_silently=False, html_message=msg_html)
...
def diz_quiz(request):
    ...
    send_mail(f'Заполнен квиз на сайте дизайна', None, 'info@specsintez-pro.ru',
              ['igor@astrapromo.ru'],
              fail_silently=False, html_message=msg_html)
...
def cb_form(request):
    ...
    send_mail(f'Запрос на обратный звонок', None, 'info@specsintez-pro.ru',
              ['igor@astrapromo.ru'],
              fail_silently=False, html_message=msg_html)
